...
/**
 * Android hardware video decoder: libwebrtc's AndroidVideoDecoder (M124), changed for streams
 * that arrive over a lossy network. Resent packets complete frames late, and several frames then
 * become decodable at once:
 *
 * - Frame info is matched to decoded frames by timestamp, not order. A decoder that skips a
 *   frame (one it cannot decode) otherwise hands each later frame the info of the one before, and
 *   the decode time it reports grows by a frame for every frame skipped. libwebrtc then keeps less
 *   and less of the playout delay for the jitter buffer, until frames arrive too late to show.
 * - A frame decoded while the texture still holds the previous one waits for it, rather than being
 *   dropped: a burst of frames after a resend is shown instead of lost.
 * - The codec runs in low-latency mode where it has one (Android 11+): each frame comes out as
 *   soon as it is decoded.
 *
 * Lives in org.webrtc for libwebrtc's package-private MediaCodec helpers.
 */
class SteadyAndroidVideoDecoder implements VideoDecoder, VideoSink {
  private static final String TAG = "SteadyAndroidVideoDecoder";

  // MediaFormat.KEY_LOW_LATENCY, API 30: by value, for apps compiling against an older SDK.
  private static final String KEY_LOW_LATENCY = "low-latency";

  // Frames decoded while the texture is busy, waiting for it. More would add latency: the oldest
  // is dropped instead.
  private static final int MAX_PENDING_TEXTURE_FRAMES = 3;

  // Frame info kept for frames in the decoder. Beyond this, the decoder has been dropping frames
  // without output, and the oldest info goes.
  private static final int MAX_FRAME_INFOS = 64;
  ...
  private static class FrameInfo {
    ...
    FrameInfo(long presentationTimeUs, long decodeStartTimeMs, int rotation) {
  ...
  // A decoded frame waiting for the texture.
  private static class PendingTextureFrame {
    final int index;
    final long presentationTimeUs;
    @Nullable final Integer decodeTimeMs;
    final int rotation;

    PendingTextureFrame(int index, long presentationTimeUs, @Nullable Integer decodeTimeMs, int rotation) {
      this.index = index;
      this.presentationTimeUs = presentationTimeUs;
  ...
  // Frames decoded while the texture was busy, oldest first. Guarded by renderedTextureMetadataLock.
  private final ArrayDeque<PendingTextureFrame> pendingTextureFrames = new ArrayDeque<>();
  ...
  SteadyAndroidVideoDecoder(MediaCodecWrapperFactory mediaCodecWrapperFactory, String codecName,
    ...
    Logging.d(TAG,
        "ctor name: " + codecName + " type: " + codecType + " color format: " + colorFormat
            + " context: " + sharedContext + " low latency: " + lowLatency);
  ...
  public VideoCodecStatus decode(EncodedImage frame, DecodeInfo info) {
    ...
    frameInfos.offer(new FrameInfo(presentationTimeUs, SystemClock.elapsedRealtime(), frame.rotation));
    while (frameInfos.size() > MAX_FRAME_INFOS) {
      frameInfos.poll();
    }
    try {
      codec.queueInputBuffer(index, 0 /* offset */, size, presentationTimeUs, 0 /* flags */);
    } catch (IllegalStateException e) {
  ...
  private Thread createOutputThread() {
    return new Thread("SteadyAndroidVideoDecoder.outputThread") {
      @Override
      public void run() {
  ...
  protected void deliverDecodedFrame() {
    ...
    try {
      ...
      FrameInfo frameInfo = takeFrameInfo(info.presentationTimeUs);
  ...
  /**
   * The info of the frame decoded at `presentationTimeUs`, and none of the frames before it: those
   * the decoder skipped.
   */
  @Nullable
  private FrameInfo takeFrameInfo(long presentationTimeUs) {
    Iterator<FrameInfo> infos = frameInfos.iterator();
    while (infos.hasNext()) {
      FrameInfo frameInfo = infos.next();
      if (frameInfo.presentationTimeUs == presentationTimeUs) {
        // By identity: the decode thread may trim the head meanwhile.
        FrameInfo head;
        do {
          head = frameInfos.poll();
        } while (head != null && head != frameInfo);
        return frameInfo;
      }
    }
    // Not a frame of ours: leave the info for the frames still in the decoder.
    return null;
  }
  ...
  private void deliverTextureFrame(final int index, final MediaCodec.BufferInfo info,
    ...
    synchronized (renderedTextureMetadataLock) {
      if (renderedTextureMetadata != null) {
        // The texture still has the previous frame: this one waits for it.
        pendingTextureFrames.add(
            new PendingTextureFrame(index, info.presentationTimeUs, decodeTimeMs, rotation));
        while (pendingTextureFrames.size() > MAX_PENDING_TEXTURE_FRAMES) {
          codec.releaseOutputBuffer(pendingTextureFrames.poll().index, false);
        }
        return;
      }
      renderToTexture(index, info.presentationTimeUs, decodeTimeMs, rotation);
    }
  }

  // Holding renderedTextureMetadataLock.
  private void renderToTexture(
      int index, long presentationTimeUs, @Nullable Integer decodeTimeMs, int rotation) {
    // Load dimensions from shared memory under the dimension lock.
    final int width;
    final int height;
    synchronized (dimensionLock) {
    ...
    renderedTextureMetadata = new DecodedTextureMetadata(presentationTimeUs, decodeTimeMs);
  ...
  public void onFrame(VideoFrame frame) {
    ...
    synchronized (renderedTextureMetadataLock) {
      ...
      // The texture is free: the next frame waiting for it goes in. The codec may be stopping on
      // the output thread meanwhile, which drops the frame.
      PendingTextureFrame next = pendingTextureFrames.poll();
      if (next != null && running && codec != null) {
        try {
          renderToTexture(next.index, next.presentationTimeUs, next.decodeTimeMs, next.rotation);
        } catch (IllegalStateException e) {
          renderedTextureMetadata = null;
          Logging.w(TAG, "Rendering a waiting frame failed", e);
        }
      }
    }
  ...
  private void releaseCodecOnOutputThread() {
    ...
    synchronized (renderedTextureMetadataLock) {
      // Their buffers go with the codec.
      pendingTextureFrames.clear();
    }
    try {
      codec.stop();
